# Flutter Shazam Kit (989 Records fork)

Fork di [ssttonn/flutter_shazam_kit](https://github.com/ssttonn/flutter_shazam_kit)
1.0.2 (licenza BSD, invariata — vedi LICENSE) che corregge il problema per
cui `SHSession.matchStreamingBuffer` risponde sempre NoMatch quando il
buffer del microfono non e' nel formato che ShazamKit si aspetta (causa
documentata da Apple, WWDC23 "Create a great ShazamKit experience"): su
iOS 17+ usa `SHManagedSession`, sotto iOS 17 converte esplicitamente il
formato con `AVAudioConverter` prima di passarlo. Interfaccia Dart
invariata rispetto all'originale — vedi CHANGELOG.md.
