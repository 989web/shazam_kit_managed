...
class MethodChannelFlutterShazamKit extends FlutterShazamKitPlatform {
  ...
  Future _nativeMethodHandler(MethodCall call) async {
    switch (call.method) {
      ...
      case "diagnostics":
        try {
          final raw = jsonDecode(call.arguments as String? ?? "{}");
          if (raw is Map<String, dynamic>) {
            onDiagnosticsCallback?.call(ShazamDiagnostics.fromMap(raw));
          }
        } catch (_) {
          // Additivo/non critico: un errore qui non deve mai propagarsi.
        }
        break;
    }
  }
